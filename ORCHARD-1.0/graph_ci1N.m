...
%  graph_ci1N.m
%
%  This functions plots a graph with several curves and an interval
...
% -----------------------------------------------------------------
function fig = graph_ci1N(x1,y1,yupp,ylow,gtitle,leg1,leg2,...
    ...
    hold all
    fh2 = fill([x1 fliplr(x1)],[yupp fliplr(ylow)],[0.9,0.9,0.9],'DisplayName',leg2);
    set(gcf,'color','white');
    ...
    set(fh1,'LineWidth',0.1);
    set(fh1,'MarkerSize',2.0);
    set(fh1,'MarkerFaceColor','w');
    set(fh1,'MarkerEdgeColor','k');
    set(fh2,'LineWidth',1.0);
    set(fh2,'EdgeColor',[0.75,0.75,0.75]);
    set(fh2,'FaceColor',[0.75,0.75,0.75]);
    %leg = legend(leg1,leg2);
	fh11 = plot(x1,y1(1,:),'-','Color','b','DisplayName',leg1);
    leg = legend([fh11(1); fh2]);
    set(leg,'FontSize',16);

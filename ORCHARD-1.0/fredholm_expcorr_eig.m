...
%  dphi_dx - (Nx x Neig) matrix with eigenfunction derivatives (in columns)
%  omega   - ( 1 x Neig) vector with charac. eq. solutions
%  xmesh   - domain discretization mesh
...
function [lambda,phi,dphi_dx,omega,xmesh,iroot,ising,iter] = ...
    ...
    if f1*f2 < 0
        ...
        if mod(iroot+1,2) ~= 0
            ...
            if abs(fv_odd) < tol
                ...
                dphi_dx(:,iroot) = -v_odd*sin(v_odd*xmesh)*NC;
            else
            ...
            if abs(fw_even) < tol
                ...
                dphi_dx(:,iroot) = w_even*cos(w_even*xmesh)*NC;
            else
